...
export default function AdminSystem() {
        ...
        try {
            const res = await api({
                ...
                params: {
                    ...
                    operations_page: operationsPage,
    ...
    }, [api, historyActor, historyPage, historyPerPage, operationsPage, operationsPerPage]);
    ...
    const operations = payload?.operations ?? { data: [], current_page: 1, last_page: 1, total: 0, per_page: operationsPerPage };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow border p-4 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="text-sm font-semibold text-gray-900">Recent operations</div>
                    <div className="text-xs text-gray-500">Total: {operations.total ?? 0}</div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <select
                        ...
                        onChange={event => {
                            setOperationsPerPage(Number(event.target.value));
                            setOperationsPage(1);
                        }}
                        className="w-full md:w-36 rounded-lg border border-gray-200 px-3 py-2 text-sm"
                    >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y">
                            {operations.data?.length ? operations.data.map(row => (
                                <tr key={`operation-${row.id}`}>
                                    <td className="px-3 py-2 text-gray-700">{new Date(row.created_at).toLocaleString()}</td>
                                    <td className="px-3 py-2 text-gray-700">{row.admin?.name || '—'}</td>
                                    <td className="px-3 py-2 text-gray-700">{row.dry_run ? 'Dry run' : 'Apply fix'}</td>
                                    <td className="px-3 py-2 text-gray-700">{row.affected_count ?? 0}</td>
                                    <td className="px-3 py-2 font-mono text-xs text-gray-600">{row.request_id || '—'}</td>
                                </tr>
                            )) : (
                                <tr>
                                    <td colSpan={5} className="px-3 py-3 text-gray-500">No operation records.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="flex items-center justify-between">
                    <div className="text-xs text-gray-500">
                        Page {operations.current_page || 1} of {operations.last_page || 1}
                    </div>
                    <div className="flex gap-2">
                        <button
                            type="button"
                            className="px-2 py-1 text-xs rounded border border-gray-300 disabled:opacity-60"
                            disabled={(operations.current_page || 1) <= 1}
                            onClick={() => setOperationsPage(Math.max(1, (operations.current_page || 1) - 1))}
                        >
                            Previous
                        </button>
                        <button
                            type="button"
                            className="px-2 py-1 text-xs rounded border border-gray-300 disabled:opacity-60"
                            disabled={(operations.current_page || 1) >= (operations.last_page || 1)}
                            onClick={() => setOperationsPage((operations.current_page || 1) + 1)}
                        >
                            Next
                        </button>
                    </div>
